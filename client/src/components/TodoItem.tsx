  ...
  return (
    ...
    >
      <div className='flex items-center gap-4 flex-1'>
        ...
        <div className="flex-1">
          <h1 className={`text-lg font-medium transition-all duration-300 ${
            todo.status ? 'text-slate-500 line-through' : 'text-slate-200'
          }`}>
            {todo.name}
          </h1>
          <span className={`text-sm transition-all duration-300 ${
            todo.status ? 'text-slate-600' : 'text-slate-400'
          }`}>
            {todo.description}
          </span>
        </div>
      ...
      <button
        ...
        className='p-2 text-slate-500 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all duration-200 opacity-0 group-hover:opacity-100 focus:opacity-100'
